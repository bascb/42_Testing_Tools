...
PATH_SRC=~/Desenvolvimento/42/projects/minishell
...
COMP=$(make -C mini_src re 2>&1)
echo "$COMP" > compile.log
if [ ! -e  mini_src/minishell ]; then
    printf "${RED} KO ${DEFAULT}|\n"
    exit
else
    printf "${GREEN} OK ${DEFAULT}|\n"
    fclean
    cp mini_src/minishell .
fi
