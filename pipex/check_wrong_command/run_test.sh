...
for i in {1..1}
do
    < ${check}/${i}.in abcd -l | wc -l > ${check}/${i}.expected
    MEM_CHECK=$(valgrind --leak-check=full --show-leak-kinds=all --track-origins=yes -s ./pipex ${check}/${i}.in "abcd -l" "wc -l" ${check}/${i}.output 2>&1)
